...
if [ $# != 2 ]; then
  echo "Usage: $0 [[--skip-db] [--skip-assets] [--local] [--exclude-tables \"table1 table2\"]] [ENV_FROM] [ENV_TO]"
  exit;
fi
...
case "$1-$2" in
  ...
  *) echo "usage: $0 [[--skip-db] [--skip-assets] [--local] [--exclude-tables \"table1 table2\"]] production development | staging development | development staging | development production | staging production | production staging" && exit 1 ;;
esac
  ...
  if [ "$SKIP_DB" = false ]; then
    ...
    if [ -n "$EXCLUDE_TABLES" ]; then
      ...
      for TABLE in "${EXCLUDE_ARRAY[@]}"; do
        EXCLUDE_OPTION="$EXCLUDE_OPTION --exclude_tables=$TABLE"
      done
    fi
    ...
    if [[ "$LOCAL" = true && $TO == "development" ]]; then
      ...
      wp db reset --yes &&
      wp "@$FROM" db export --default-character-set=utf8mb4 $EXCLUDE_OPTION - | wp db import - &&
      ...
      wp "@$TO" db reset --yes &&
      wp db export --default-character-set=utf8mb4 $EXCLUDE_OPTION - | wp "@$TO" db import - &&
      wp "@$TO" search-replace "$FROMSITE" "$TOSITE" --all-tables-with-prefix
    else
      wp "@$TO" db export --default-character-set=utf8mb4 &&
      wp "@$TO" db reset --yes &&
      wp "@$FROM" db export --default-character-set=utf8mb4 $EXCLUDE_OPTION - | wp "@$TO" db import - &&
      wp "@$TO" search-replace "$FROMSITE" "$TOSITE" --all-tables-with-prefix
    fi
  fi
