                ...
                @foreach ($slider as $slide)
                    <li class="glide__slide" formato="{!! $slide['formato'] !!}">
                        <a href="{!! $slide['url'] !!}" class="prod relative block">
                            <div class="bg-hover absolute bottom-0 top-0 hidden bg-white p-8 uppercase md:block">
                                <div class="datos leading-tight">
                                    ...
                                    @if ($slide['product_type'] == 'simple')
                                        @if ($slide['has_format'])
                                            <p class="mb-4 font-bold tracking-widest text-black">{{ $slide['format'] }}
                                                cm</p>
                                        @endif
                                        @if ($slide['has_sale_prize'])
                                            <p class="mb-4 font-bold text-red-600 line-through">
                                                {{ $slide['regular_price'] }} €</p>
                                            <p class="mb-4 font-bold tracking-widest text-black">
                                                {{ $slide['sale_prize'] }} €</p>
                                        @else
                                            <p class="mb-4 font-bold tracking-widest text-black">
                                                {{ $slide['regular_price'] }} €</p>
                                    ...
                                    @elseif($slide['product_type'] == 'variable')
                                        @foreach ($slide['variaciones'] as $v)
                                            @if ($v['sale_price'] != '')
                                                <p class="font-bold tracking-widest text-black">{{ $v['format'] }} cm
                                                </p>
                                                <p class="mb-4 font-bold tracking-widest text-black"><span
                                                        class="text-red-600 line-through">{{ $v['regular_price'] }}
                                                        €</span> {{ $v['sale_price'] }} €</p>
                                            @else
                                                <p class="mb-4 font-bold tracking-widest text-black">{{ $v['format'] }}
                                                    cm {{ $v['regular_price'] }} €</p>
                                            @endif
                                        @endforeach
                                    @endif
                            ...
                            @if (array_key_exists('img', $slide))
                                <img class="relative" src="{!! $slide['img']['url'] !!}" srcset="{!! $slide['srcset'] !!}"
                                    sizes="(max-width: 640px) 100vw, 80vw" alt="">
                            @endif
                        </a>
                    </li>
                @endforeach
