...
<div class="cart_totals {!! WC()->customer->has_calculated_shipping() ? 'calculated_shipping' : '' !!} font-sans">
    ...
    <table cellspacing="0" class="shop_table shop_table_responsive w-full">
        ...
        @foreach (WC()->cart->get_coupons() as $code => $coupon)
            <tr class="cart-discount coupon-{{ sanitize_title($code) }}">
                ...
                <td data-title="{{ esc_attr(wc_cart_totals_coupon_label($coupon, false)) }}">@php
                wc_cart_totals_coupon_html($coupon); @endphp</td>
            </tr>
        @endforeach
