...
export function anadirAlCarroVariable() {
  ...
  let size = $('#ftbs_variationsTableRow_0').attr('attributevalue'),
    paFormat = $('#pa_format'),
    rows = $('.ftbs_variationsTableRow'),
    quantityInputsAdd = $('.ftbs_variationsTableRowColumn_quantityInput_add'),
    quantityInputsRemove = $(
      '.ftbs_variationsTableRowColumn_quantityInput_remove'
    );
  ...
  function ftbs_singleProductAttributeInteraction() {
    ...
    $('.ftbs_variationsTableRow').addClass(
      'ftbs_variationsTableRow_unselected'
    );
    $('#ftbs_variationsTableRow_' + idNumber).removeClass(
      'ftbs_variationsTableRow_unselected'
    );
    ...
    $('#ftbs_variationsTableRowColumn_radioInput_' + idNumber).attr(
      'name',
      'attribute_pa_format'
    );
    $('#ftbs_variationsTableRowColumn_radioInput_' + idNumber).prop(
      'checked',
      true
    );

    var variation_id = $(
      '#ftbs_variationsTableRowColumn_radioInput_' + idNumber
    ).attr('variation_id');
    ...
    var quantityIncrease = $(
      '#ftbs_variationsTableRowColumn_quantityInput_add'
    );
    ...
    /** Get empty input **/
    var quantityInput_inactive = $(
      '#ftbs_variationsTableRowColumn_quantity_' +
        idNumber +
        ' .ftbs_variationsTableRowColumn_quantityInput_inactive'
    );
    var quantityIncrease_inactive = $(
      '#ftbs_variationsTableRowColumn_quantity_' +
        idNumber +
        ' .ftbs_variationsTableRowColumn_quantityInput_add_inactive'
    );
    var quantityDecrease_inactive = $(
      '#ftbs_variationsTableRowColumn_quantity_' +
        idNumber +
        ' .ftbs_variationsTableRowColumn_quantityInput_remove_inactive'
    );
    ...
    /** Get destiny element **/
    var insertTo_input = $(
      '#ftbs_variationsTableRowColumn_quantity_' + idNumber
    );
    var insertTo_increase = $(
      '#ftbs_variationsTableRowColumn_quantity_' + idNumber
    );
    var insertTo_decrease = $(
      '#ftbs_variationsTableRowColumn_quantity_' + idNumber
    );
    ...
    $('.ftbs_variationsTableRowPadContainer').addClass(
      'ftbs_variationsTableRowPadContainer_inactive'
    );
    $(insertTo_input)
      .parent()
      .removeClass('ftbs_variationsTableRowPadContainer_inactive');

    /* Reset */
    quantityInput.value = 1;
    $('input[name="quantity"]').val(1);
    $(
      '.ftbs_variationsTableRowColumn_quantityInput:not(.ftbs_variationsTableRowColumn_quantityInput_inactive)'
    ).val(1);
  }
  ...
  function ftbs_clickOnProductVariationRow() {
    ...
    if (
      $('#ftbs_variationsTableRowColumn_quantity_' + id).find(
        '#ftbs_variationsTableRowColumn_quantityInput'
      ).length
    ) {
      return false;
    }
  ...
  for (var i = 0; i < quantityInputsAdd.length; i++) {
    quantityInputsAdd[i].addEventListener(
      'click',
      ftbs_product_quantity_increase
    );
  }
  ...
  for (var i = 0; i < quantityInputsRemove.length; i++) {
    quantityInputsRemove[i].addEventListener(
      'click',
      ftbs_product_quantity_decrease
    );
  }
  ...
  $('.ftbs_variationsTableRowColumn_radioInput').change(
    ftbs_singleProductAttributeInteraction
  );

  $('.ftbs_singleProductQuantityInteraction').change(
    ftbs_singleProductQuantityInteraction
  );
}
